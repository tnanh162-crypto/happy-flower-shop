...
const DATA_DIR = process.env.VERCEL
  ? path.join("/tmp", "flower-shop-data")
  : path.join(process.cwd(), "data");
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

const localDbFile = path.join(process.cwd(), "data", "shop.db");
const dbFile = path.join(DATA_DIR, "shop.db");
if (process.env.VERCEL && !fs.existsSync(dbFile) && fs.existsSync(localDbFile)) {
  fs.copyFileSync(localDbFile, dbFile);
}

const globalForDb = globalThis;
const db = globalForDb.__flowerShopDb || new Database(dbFile);
